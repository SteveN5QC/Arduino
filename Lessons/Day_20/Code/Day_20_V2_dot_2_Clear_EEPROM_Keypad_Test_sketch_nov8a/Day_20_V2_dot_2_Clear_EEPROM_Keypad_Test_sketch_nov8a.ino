...
void savePINToEEPROM() {
    ...
    delay(10);  // Add delay to ensure each byte is written
    for (int i = 0; i < length; i++) {
        EEPROM.write(eepromStartAddress + 1 + i, currentPIN[i]);
        delay(10);
    }
    EEPROM.write(magicNumberAddress, magicNumber); // Save the magic number
    delay(10); // Final delay after writing all data
    
    // Debugging: Confirm write
    Serial.print("Saved PIN Length: ");
    Serial.println(length);
    Serial.print("Magic Number Saved: ");
    Serial.println(EEPROM.read(magicNumberAddress));
}
...
bool loadPINFromEEPROM() {
    ...
    // Debugging: Confirm load
    Serial.print("Loaded PIN Length: ");
    Serial.println(length);
    Serial.print("Magic Number Loaded: ");
    Serial.println(EEPROM.read(magicNumberAddress));
