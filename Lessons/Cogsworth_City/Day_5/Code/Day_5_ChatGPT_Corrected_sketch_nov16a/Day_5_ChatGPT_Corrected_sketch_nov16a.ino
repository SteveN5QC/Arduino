const int switchPin1 = 2;    // Switch 1 is connected to pin 2
...
void loop() {
  ...
  if (switchState1 != lastSwitchState1) {
    if (switchState1 == HIGH) {
      ledState1 = !ledState1;  // Toggle LED state (corrected assignment operator)
  ...
  if (switchState3 != lastSwitchState3) {
    if (switchState3 == HIGH) {
      ledState2 = !ledState2;  // Toggle LED state (corrected assignment operator)
